...
export const UpdateProduct = async (productId: number, accessToken: string) => {
  try {
    const response = await fetch(ApiRoutes.Product_update(productId), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export const deleteProduct = async (accessToken: string, list: Number[]) => {
  try {
    const response = await fetch(ApiRoutes.Product_delete, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${accessToken}`, 
